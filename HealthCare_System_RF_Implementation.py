...
with st.sidebar:
    selection = option_menu('Menu', 
                            ...
                            icons = ['tree-fill'],
                            default_index = 0)
    st.write('This app was developed for the diagnosis of diabetes given the relevant \
             parameters.')
...
if selection == 'RF Prediction':
    ...
    with col1:
        Pregnancies = st.number_input('No of Pregnacies', min_value=0, key='Pregnancies')
    ...
    with col2:
        if st.button('Outcome'):
            outcome = rf.predict([[Pregnancies, BloodPressure, BMI, Age]])
            if outcome[0] == 1:
                prediction = 'DIABETIC'
            else:
